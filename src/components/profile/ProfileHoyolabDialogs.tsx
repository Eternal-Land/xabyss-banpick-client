    ...
    <>
      <Dialog open={isDialogOpen} onOpenChange={onDialogOpenChange}>
        ...
        <DialogContent className="max-h-[80vh] min-w-[70rem] overflow-y-auto bg-transparent bg-linear-45 from-white/5 to-white/10 backdrop-blur-md sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t(LocaleKeys.profile_hoyolab_dialog_title)}</DialogTitle>
            <DialogDescription>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={onConfirmOpenChange}>
        <DialogContent className="max-w-lg bg-transparent bg-linear-45 from-white/5 to-white/10 backdrop-blur-md">
          <DialogHeader>
            <DialogTitle>{t(LocaleKeys.profile_hoyolab_confirm_title)}</DialogTitle>
            <DialogDescription>
              {t(LocaleKeys.profile_hoyolab_confirm_description)}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label className="text-sm text-white/70" htmlFor="hoyolab-uid">
                {t(LocaleKeys.profile_hoyolab_uid_label)}
              </Label>
              <Input
                id="hoyolab-uid"
                placeholder={t(LocaleKeys.profile_hoyolab_uid_placeholder)}
                value={hoyoUid}
            ...
            <div className="space-y-2">
              <Label className="text-sm text-white/70">
                {t(LocaleKeys.profile_hoyolab_server_label)}
              </Label>
              <Select value={hoyoServer} onValueChange={onHoyoServerChange}>
                <SelectTrigger className="w-full">
                  <SelectValue
                    placeholder={t(
                      LocaleKeys.profile_hoyolab_server_placeholder,
                    )}
                  />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="os_asia">
                    {t(LocaleKeys.profile_hoyolab_server_asia)}
                  </SelectItem>
                  <SelectItem value="os_usa">
                    {t(LocaleKeys.profile_hoyolab_server_america)}
                  </SelectItem>
                  <SelectItem value="os_euro">
                    {t(LocaleKeys.profile_hoyolab_server_europe)}
                  </SelectItem>
                  <SelectItem value="os_cht">
                    {t(LocaleKeys.profile_hoyolab_server_taiwan)}
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="rounded-lg border border-yellow-400/40 bg-yellow-400/10 px-3 py-2 text-xs text-yellow-200">
              {t(LocaleKeys.profile_hoyolab_sync_warning)}
            </div>
          </div>
          <DialogFooter>
            <DialogClose asChild>
            ...
            <Button onClick={onConfirmSync} disabled={!isSyncReady || isSyncPending}>
              {isSyncPending
                ? t(LocaleKeys.profile_hoyolab_sync_submit_button)
                : t(LocaleKeys.profile_hoyolab_confirm_button)}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={isResultOpen} onOpenChange={onResultOpenChange}>
        <DialogContent className="max-w-lg bg-transparent bg-linear-45 from-white/5 to-white/10 backdrop-blur-md">
          <DialogHeader>
            <DialogTitle>
              {syncResult === "success"
                ? t(LocaleKeys.profile_hoyolab_result_success_title)
                : t(LocaleKeys.profile_hoyolab_result_error_title)}
            </DialogTitle>
            <DialogDescription>
              {syncResult === "success"
                ? t(LocaleKeys.profile_hoyolab_result_success_description)
                : t(LocaleKeys.profile_hoyolab_result_error_description)}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="flex w-full items-center justify-center px-4 py-6">
              {syncResult === "success" ? (
                <img
                  ...
                  alt={t(LocaleKeys.profile_hoyolab_result_success_title)}
                ...
                <img
                  ...
                  alt={t(LocaleKeys.profile_hoyolab_result_error_title)}
                  className="max-h-48 w-full object-contain"
                />
              )}
            </div>
          ...
          <DialogFooter>
            <DialogClose asChild>
              <Button variant="outline">
                {t(LocaleKeys.profile_hoyolab_result_close)}
              </Button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
