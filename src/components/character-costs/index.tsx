...
export default function CharacterCostsTab() {
	...
	const listCharacterCostsQuery = useQuery({
		...
		queryFn: async () => {
			...
			let next = undefined;
			do {
				const res = await characterCostsApi.listCharacterCosts({
					...filter,
					startId: next,
				});
				data.push(...(res.data || []));
	...
	const saveMessage = isSaving
		? t(
				getTranslationToken(
					"character-costs",
					characterCostsLocaleKeys.character_costs_saving,
				),
			)
		: hasSaved
			? t(
					getTranslationToken(
						"character-costs",
						characterCostsLocaleKeys.character_costs_saved,
					),
				)
			: null;
											...
											{elementOptions.map((option) => {
												...
												return (
													<DropdownMenuCheckboxItem
														key={option.value}
														checked={
															filter.element?.includes(elementValue) ?? false
														}
				...
				<div className="flex items-center gap-6">
					...
					>
						{t(
							getTranslationToken(
								"character-costs",
								characterCostsLocaleKeys.character_costs_sync,
							),
						)}
					</Button>
				</div>
				...
				<TableHeader>
					<TableRow>
						...
						{Array.from({ length: 7 }).map((_, index) => (
							<TableHead key={index} className="text-center w-15">
								C{index}
							</TableHead>
						))}
					</TableRow>
						...
						? Array.from({ length: 6 }).map((_, index) => (
								<TableRow key={`character-cost-skeleton-${index}`}>
									...
									{Array.from({ length: 7 }).map((_, costIndex) => (
										<TableCell
											key={`character-cost-skeleton-${index}-${costIndex}`}
										>
