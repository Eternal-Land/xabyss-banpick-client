...
function RouteComponent() {
  ...
  const { data: accountCharactersResponse, refetch: refetchAccountCharacters } = useQuery({
  ...
  >({
    ...
    onSuccess: () => {
      toast.success(t(LocaleKeys.profile_hoyolab_sync_success));
    ...
    onError: (error) => {
      toast.error(
        ...
        t(LocaleKeys.profile_hoyolab_sync_error),
      );
      setIsSyncConfirmOpen(false);
      setSyncResult("error");
      setIsSyncResultOpen(true);
  ...
  >({
    mutationFn: (characterId) =>
      accountCharactersApi.createAccountCharacter({
        ...
        activatedConstellation: constellation ? Number(constellation) : undefined,
        isOwned: true,
      }),
    onSuccess: () => {
      toast.success(t(LocaleKeys.profile_add_character_success));
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message ||
        t(LocaleKeys.profile_add_character_error),
      );
    },
  });
  ...
  >({
    ...
    onSuccess: () => {
      toast.success(t(LocaleKeys.profile_edit_character_success));
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message ||
        t(LocaleKeys.profile_edit_character_error),
      );
    },
  });
  ...
  >({
    ...
    onSuccess: () => {
      toast.success(t(LocaleKeys.profile_remove_character_success));
    ...
    onError: (error) => {
      toast.error(
        error.response?.data.message ||
        t(LocaleKeys.profile_remove_character_error),
      );
    },
  });
        ...
        <div className="min-h-screen text-white">
          <div className="mx-auto w-full max-w-6xl px-6 pb-12 pt-24">
            ...
            <div className="mt-8 grid gap-4 md:grid-cols-10">
              {(accountCharacters.length ? accountCharacters : defaultCharacters).map(
                (character, index) => {
