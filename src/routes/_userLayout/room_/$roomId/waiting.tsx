    ...
    <div className="min-h-screen max-w-screen">
      <div className="grid grid-cols-4 gap-4 h-dvh p-4">
        <div className="flex flex-col h-full justify-center items-center gap-2">
          <h1 className="text-4xl font-bold text-sky-400 capitalize">{tMatch(matchLocaleKeys.match_waiting_blue_player)}</h1>

          <div className="w-32 h-32 rounded-full object-cover mt-4 flex items-center justify-center overflow-hidden">
            {bluePlayer?.avatar ? (
              <img src={bluePlayer.avatar} alt="Blue Player Avatar" className="w-full h-full" />
            ) : (
              <img src={IconAssets.EMPTY_CHARACTER_ICON} alt="Blue Player Avatar" className="w-12 h-12" />
            )}
          </div>

          <span className="text-xl mt-2">{bluePlayer?.displayName ?? tMatch(matchLocaleKeys.match_waiting_connecting)}</span>
          <span className="text-xl mt-2">{tMatch(matchLocaleKeys.match_waiting_uid_label)}: {bluePlayer?.ingameUuid ?? '-'}</span>
        </div>
        ...
        <div className="col-span-2 flex flex-col h-full justify-between items-center py-4">
          <div className="host-area w-full flex justify-center items-center gap-4">
            {match.host?.avatar ? (
              <img src={match.host.avatar} alt="Host Avatar" className="w-20 h-20 rounded-full object-cover" />
            ) : (
        ...
        <div className="flex flex-col h-full justify-center items-center">
          <h1 className="text-4xl font-bold text-red-600 capitalize">{tMatch(matchLocaleKeys.match_waiting_red_player)}</h1>
          <div className="w-32 h-32 rounded-full object-cover mt-4 flex items-center justify-center border-2 border-dashed border-gray-400 overflow-hidden">
            {redPlayer?.avatar ? (
              <img src={redPlayer.avatar} alt="Red Player Avatar" className="w-full h-full" />
            ) : (
              <img src={IconAssets.EMPTY_CHARACTER_ICON} alt="Red Player Avatar" className="w-12 h-12" />
            )}
          </div>
          <span className="text-xl mt-2">{redPlayer?.displayName ?? tMatch(matchLocaleKeys.match_waiting_connecting)}</span>
